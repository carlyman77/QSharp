...
{
    /// <summary>
    /// Describes a triple-qubit command that may be executed against a Register.
    /// </summary>
    public class TripleQubitCommand : Command
    {
        /// <summary>
        /// Constructs a new instance of the TripleQubitCommand type.
        /// </summary>
        /// <param name="commandValue">The text value of the command. This is usually corresponds to a gate or matrix operation.</param>
        protected TripleQubitCommand(string commandValue)
            : base(commandValue)
        {
        }
        ...
        /// </param>
        protected TripleQubitCommand(string commandValue, string shortHand)
        ...
        protected TripleQubitCommand(string commandValue, string shortHand, int control0, int control1, int target)
        ...
        private int _control0;
        private int _control1;
        private int _target;

        /// <summary>
        /// Gets the first control qubit index.
        /// </summary>
        public int Control0
        {
            get
        ...
        /// <summary>
        /// Gets the second control qubit index.
        /// </summary>
        public int Control1
        {
            get
        ...
        public override CommandResult Execute(string value, Register register)
        {
            ...
            else
            {
                ...
                if ((isValidValues == true) && (iValues.Length == 3))
                {
                    _control0 = iValues[0];
                    _control1 = iValues[1];
                    _target = iValues[2];

                    if ((iValues.Length <= register.Qubits.Length) && (iValues.Min(m => (m)) >= 0) && ((iValues.Max(m => (m)) - 1) < register.Qubits.Length) && (_control0 != _control1) && (_control0 != _target) && (_control1 != _target))
                    {
                    }
                    else
                    {
                        commandResult = new CommandResult(String.Format("Arguments for the {0} command were out of the expected range. Provided values were Control0 = {1}, Control1 = {2} and Target = {3}.", CommandValue, _control0, _control1, _target), false);
                    }
                }
                else
                {
